"""
Script to run the single and multi scenario simulation for the CTU dataset
"""
...
def write_to_csv(results, klthreshold, states, case, lsh=True):
    workbook_path = "../test_results/ctu/{}/hosts".format(features_dir)
    if not os.path.exists(workbook_path):
        os.makedirs(workbook_path)

    if lsh:
        file_workbook = workbook_path + "/1SD_3LSHs_ctu_lsh_0.01_results_largestblue_0_{}_{}_{}.xlsx".format(
            window,
            stride,
            case)
    else:
...
def train_model(malicious_traces_file, difference):
    ...
    parser.add_argument('--vectordimension', '-v', help="vectordimension", type=str, default='100') #exeprioments done with 50
...
# runs the evaluation for the single-scenario simulation
...
# runs the evaluation for the multi-scenario simulation

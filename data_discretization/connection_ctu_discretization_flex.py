...
def discretize(data, data_dir, scenario, window_size, stride_size, percentile_num, case='train', conf=False):
    ...
    initial_stride_size = stride_size
    for selected_data in all_connections:
        ...
        if date_median == 0:
            date_median += 1
        biggest_date_diff = date_diff(dates[0], dates[-1])
        ...
        first_date = selected_data.index.tolist()[0]
        last_date = selected_data.index.tolist()[-1]
        ...
        prev_window = []
        per = 0.1
        window_size = int(initial_window_size * date_median)
        stride_size = int(initial_stride_size * date_median)

        print('Window = {}, Stride = {}'.format(str(window_size), str(stride_size)))

        starting_date = selected_data.index.tolist()[0]
        ending_date = starting_date + datetime.timedelta(milliseconds=window_size)
        if ending_date < last_date:
            end_date_found = False
        else:
            end_date_found = True
        while True:
            if date_diff(first_date, starting_date) > biggest_date_diff * per:
            ...
            window_lenth = len(data_subset)
            if window_lenth > 1.1 * initial_window_size:
                window_size -= 1000
                stride_size -= 1000
            elif window_lenth < 0.9 * initial_window_size:
                window_size += 1000
                stride_size += 1000
            print('Current window length: {}, window size: {}'.format(window_lenth, window_size))
            ...
            if end_date_found:
                break
            starting_date += datetime.timedelta(milliseconds=stride_size)
            ending_date = starting_date + datetime.timedelta(milliseconds=window_size)
...
def discretize_for_multiple_scenarios():
    ...
    conf_dir = 'discretized_data_1000_500_flex/ctu_13/connection/multiple_scenarios/configuration_data/'
    extract_conf_traces(window_size, stride_size, all_conf_data, percentile_num, conf_dir, '')
    for s in scenarios:
        scenario = 'scenario_{}'.format(s)
        dataset = 'CTU-Malware-Capture-Botnet-{}'.format(s)
        data_dir = 'discretized_data_1000_500_flex/ctu_13/connection/multiple_scenarios/{}/{}-{}_median__traces_ctu_{}'.format(
            scenario,
            window_size,
            stride_size,
            s)

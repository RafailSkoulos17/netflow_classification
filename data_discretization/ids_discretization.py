...
if sys.argv[0].split('/')[-1] == os.path.basename(__file__):
    from utils import rolling_window, lists_identical, lists_identical, netflow_encoding, find_percentile, date_diff, \
        split_data
else:
    from pypackage.flexfringe.utils import rolling_window, lists_identical, lists_identical, netflow_encoding, \
        find_percentile, date_diff, split_data
import datetime
...
def read_and_process_data(dataset):
    ...
    #         if v['src_ip'] not in double_labeled_ips:
    #             if v['src_ip'] not in t:
    #                 t[v['src_ip']] = v['label']
    #             else:
    #                 if t[v['src_ip']] != v['label']:
    #                     double_labeled_ips[v['src_ip']] = 1
    #     # infected_ips[filename] = {}
    #     # for k, v in double_labeled_ips.items():
    #     #     infected_ips[filename] += [double_labeled_ips[v]]
    #     if mode == 'train':
    #         for ip in double_labeled_ips.keys():
    #             data = data[~((data['src_ip'] == ip) & (data['label'] == 0))]

    return data


def discretize(data, data_dir, window_size, stride_size, percentile_num,
               conf=False):
...
def extract_traces(dataset, data_dir, window_size, stride_size, percentile_num, multiple=False):
    ...
    if not multiple:
        ...
        discretize(configuration_data.copy(), data_dir, window_size, stride_size,
                   percentile_num, conf=True)

    discretize(data.copy(), data_dir, window_size, stride_size,
               percentile_num, conf=True)


def extract_conf_traces(window_size, stride_size, all_conf_data, percentile_num, conf_data_dir, dataset):
    conf_data_dir = conf_data_dir + '/' + dataset.split('/')[-1].rstrip('.csv')
    ...
    for subdir, dirs, files in os.walk(rootdir):
        for filename in files:
            ...
            if filename == benign_scenario:
                ...
                data_dir = 'discretized_data/ids/single_scenario'.format(filename.rstrip('.csv'))
...
def discretize_multiple_scenarios():
    ...
    conf_dir = 'discretized_data/ids/multiple_scenarios/configuration_data/'
    ...
    for subdir, dirs, files in os.walk(rootdir):
        for filename in files:
            ...
            if filename == 'Monday-WorkingHours.pcap_ISCX.csv':
                ...
                data_dir = 'discretized_data/ids/multiple_scenarios{}'.format(filename.rstrip('.csv'))
                file_path = subdir + filename

...
from pypackage.flexfringe.utils import preprocess_bidirectional_data, rolling_window, lists_identical, netflow_encoding, \
    find_percentile, remove_background, date_diff, split_data, foo
...
def discretize(data, data_dir, scenario, window_size, stride_size, percentile_num, case='train', conf=False):
    ...
    for selected_data in all_connections:
        ...
        print('Window = {}, Stride = {}'.format('100 flows', '1 flow'))
        windows = []
        # selected_data.rolling(window='1s').apply(lambda x: foo(x, windows))
        selected_data.rolling(window=100).apply(lambda x: foo(x, windows))
...
def discretize_for_multiple_scenarios():
    ...
    conf_dir = 'discretized_data_fixed_100_flows/ctu_13/connection/multiple_scenarios/configuration_data/'
    extract_conf_traces(window_size, stride_size, all_conf_data, percentile_num, conf_dir, '')
    for s in scenarios:
        scenario = 'scenario_{}'.format(s)
        dataset = 'CTU-Malware-Capture-Botnet-{}'.format(s)
        data_dir = 'discretized_data_fixed_100_flows/ctu_13/connection/multiple_scenarios/{}/{}-{}_median__traces_ctu_{}'.format(
            scenario,
            window_size,
            stride_size,
            s)

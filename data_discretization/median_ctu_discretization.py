...
from pypackage.flexfringe.utils import preprocess_bidirectional_data, rolling_window, lists_identical, netflow_encoding, \
    find_percentile, date_diff, split_data, remove_background
...
# selected_features = ['duration', 'direction', 'protocol', 'packets', 'total_bytes']
...
def find_median(conf_data_dict):
    if not isinstance(conf_data_dict, dict):
        conf_data_dict = {"1": conf_data_dict}
    all_means = []
    for k, data in conf_data_dict.items():
        data = data.set_index(['date'])
        data = data.sort_index()
        medians = []
        all_hosts = [group for _, group in data.groupby(['src_ip'])]
        for host in all_hosts:
            dates = host.index.tolist()
            if len(dates) < 2:
                continue
            date_diffs = [date_diff(s, t) for s, t in zip(dates, dates[1:])]
            date_diffs = [d for d in date_diffs if d <= 1000 and d != 0]
            if date_diffs:
                medians += [int(np.median(date_diffs))]
        if medians:
            all_means += [int(np.mean(medians))]
    return int(np.mean(all_means))


def extract_conf_traces(window_size, stride_size, conf_data_dict, percentile_num, conf_data_dir, scenario,
                        case='train'):
    median = find_median(conf_data_dict)
    if isinstance(conf_data_dict, dict):
        all_conf_data = pd.DataFrame()
...
def discretize_for_single_scenario():
    ...
    for s in scenarios:
        ...
        data_dir = '_'.join(
            selected_features) + '_discretized_data_{}median_{}median/ctu_13/single_scenario/{}/{}-{}_traces_ctu_{}'.format(
            window_size,
            stride_size, scenario,
            window_size,
            stride_size,
            s)
        ...
        configuration_data, data = split_data(data, normal_ips)
        configuration_data = configuration_data.sort_index()
        data = data.sort_index()
        percentile_num = get_precentiles(configuration_data)
        ...
        extract_traces(data, data_dir, scenario, window_size * median,
                       stride_size * median, {}, percentile_num, case='test', multiple=False)
...
def discretize_for_multiple_scenarios():
    ...
    for s in scenarios:
        dataset = 'CTU-Malware-Capture-Botnet-{}'.format(s)
        normal_ips = ip_dict["scenario_" + str(s)]['normal_ips']
        if s in training_sets:
            all_data = read_and_process_data(dataset)
            conf_data, data = split_data(all_data, normal_ips)
    ...
    conf_dir = '_'.join(
        selected_features) + '_discretized_data_{}median_{}median/ctu_13/multiple_scenarios/configuration_data/'.format(
        window_size,
        stride_size)
    median = extract_conf_traces(window_size, stride_size, conf_data_dict, percentile_num, conf_dir, '', case='test')
    for s in scenarios:
        scenario = 'scenario_{}'.format(s)
        dataset = 'CTU-Malware-Capture-Botnet-{}'.format(s)
        data_dir = '_'.join(
            selected_features) + '_discretized_data_{}median_{}median/ctu_13/multiple_scenarios/{}/{}-{}_traces_ctu_{}'.format(
            window_size,
            stride_size,
            scenario,
            window_size,

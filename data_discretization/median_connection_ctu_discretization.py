...
def discretize(data, data_dir, scenario, window_size, stride_size, percentile_num, case='train', conf=False):
    ...
    if case == 'train':
        ...
        all_connections = [group for _, group in data.groupby(['src_ip', 'dst_ip']) if len(group) > 100]
...
def find_median(conf_data_dict):
    ...
    for k, data in conf_data_dict.items():
        ...
        for connection in all_connections:
            ...
            # date_diffs = [d for d in date_diffs]
            if date_diffs:
...
def discretize_for_single_scenario():
    ...
    for s in scenarios:
        ...
        data_dir = 'discretized_data_{}median_{}median/ctu_13/connection/single_scenario/{}/{}/{}-{}_traces_ctu_{}'.format(
            window_size,
            stride_size,
            features_dir,
            scenario,
...
def discretize_for_multiple_scenarios():
    ...
    conf_dir = 'discretized_data_{}median_{}median_notall_direction_timediff/ctu_13/connection/multiple_scenarios/configuration_data/'.format(
        window_size,
        stride_size)
    median = extract_conf_traces(window_size, stride_size, conf_data_dict, percentile_num, conf_dir, '', case='train')
    for s in scenarios:
        scenario = 'scenario_{}'.format(s)
        dataset = 'CTU-Malware-Capture-Botnet-{}'.format(s)
        data_dir = 'discretized_data_{}median_{}median_notall_direction_timediff/ctu_13/connection/multiple_scenarios/{}/{}-{}_traces_ctu_{}'.format(
            window_size,
            stride_size,
            scenario,
            window_size,

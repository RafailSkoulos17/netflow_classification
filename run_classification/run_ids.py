...
from pypackage.flexfringe.data_analysis_and_discretization import split_data
...
def write_to_csv(results, klthreshold, states, case, lsh=True):
    if lsh:
        file_workbook = "../results/ids/ids_0.1_lsh_results_{}_{}_{}.xlsx".format(window, stride, case)
    else:
        file_workbook = "../results/ids/ids_trigram_results_{}_{}_{}.xlsx".format(window, stride, case)
...
def train_model(malicious_traces_file, difference):
    ...
    parser.add_argument('--tablesize', '-S', help="tablesize", type=str, default='50')
...
def evaluate_multiple_scenarios(models, all_hosts, configuration_traces_files, malicious_traces_files, lsh=True):
    ...
    #     # for configuration_traces_file in configuration_traces_files:
    #     # acceptance_ratios += [get_profiling_threshold(model, configuration_traces_file)]
    #     configuration_traces_file = configuration_traces_files[0]
    #     acceptance_ratios += [get_profiling_threshold(model, configuration_traces_file)]
    #     acceptance_ratio += [np.mean(acceptance_ratios)]
    #     # acceptance_ratio = np.mean(acceptance_ratios) - 2 * np.std(acceptance_ratios)
    #     print("ACCEPTANCE RATIO FOR SCENARIO {} IS: {}\n".format(scenario, acceptance_ratio))
    ...
    #         evaluation_results += [profiling_evaluation(model, host, acceptance_ratio[i])]
    ...
    for host in all_hosts:
        ...
        for i, model in enumerate(models):
            # threshold = get_selectivity_threshold(model, configuration_traces_files, malicious_traces_files[i])
            evaluation_results += [error_based_classification(model, host,
                                                              malicious_traces_files[i], threshold[i])]
        if np.sum(evaluation_results) >= 1:
            evaluation_result = 1
        else:
            evaluation_result = 0
    ...
    for host in all_hosts:
        ...
        evaluation_results = []
        for i, model in enumerate(models):
            evaluation_results += [fingerprint_based_classification(model, host,
                                                                    malicious_traces_files[i],
...
def run_single_scenario_evaluation(lsh=True):
    ...
    global data_dir
    global scenario
    rootdir = "../data/discretized_data/ids/single_scenario"
    for subdir, dirs, files in os.walk(rootdir):
        if len(subdir.split('/')) != 5:
            continue
        dir = [d for d in dirs if 'ctu' in d][0]
        # if subdir != 'data/ctu_13/bidirectional/scenario_46':
        #     continue
...
def run_multiple_scenarios_evaluation(lsh=True):
    ...
    global data_dir
    global scenario
    rootdir = "../data/discretized_data/ids/multiple_scenarios"
    ...
    for subdir, dirs, files in os.walk(rootdir):
        ...
        all_hosts = glob.glob(subdir + "/*traces")
        malicious_traces_files[sheet_name] = [f for f in all_hosts if 'infected' in f]
    ...
    for subdir, dirs, files in os.walk(rootdir):
        if len(subdir.split('/')) != 3:
            continue
        sheet_name = subdir.split('/')[-1]
        current_malicious_traces_files = malicious_traces_files[sheet_name][:2]
    ...
    for subdir, dirs, files in os.walk(rootdir):
        ...
        window = str(window) + 'median'
        stride = str(stride) + 'median'
        all_hosts = [f for f in glob.glob(subdir + "/*traces") if 'infected' in f] + malicious_traces_files[sheet_name][
                                                                                     2:]
        evaluate_multiple_scenarios(models, all_hosts, configuration_traces_files, given_malicious_trace, lsh=lsh)

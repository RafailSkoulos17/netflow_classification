...
def write_to_csv(results, klthreshold, states, case, lsh=True):
    ...
    workbook_path = "../results/ctu/{}/hosts".format(features_dir)
    if not os.path.exists(workbook_path):
        os.makedirs(workbook_path)

    if lsh:
        file_workbook = workbook_path + "/STREAM_majority_3LSHs_RH_{}_ctu_0.05_lsh_results_{}_{}_{}.xlsx".format(pre,
                                                                                                                window,
                                                                                                                stride,
                                                                                                                case)
    else:
        file_workbook = workbook_path + "/majority_trigram_results_{}_{}_{}_{}.xlsx".format(pre, window,
                                                                                            stride,
                                                                                            case)
    #
    # if lsh:
    #     file_workbook = "../results/ctu/3LSHs_RH_{}_ctu_0.05_lsh_results_{}_{}_{}.xlsx".format(pre, '20ms',
    #                                                                                             '1_flow',
    #                                                                                             case)
    # else:
    #     file_workbook = "../results/ctu/{}_ctu_trigram_results_{}_{}_{}.xlsx".format(pre, '20ms',
    #                                                                                             '1_flow',
    #                                                                                             case)
...
def train_model(malicious_traces_file, difference):
    ...
    parser.add_argument('--width', '-W', help="bin width", type=str, default='30')
    ...
    states = flexfringe(args.train_file, M=args.mode, h=args.heuristic_name, d=args.data_name, q=args.state_count,
                        y=args.symbol_count, a=args.largestblue, f=args.finalred, D=args.delta, e=args.epsilon,
...
def evaluate_multiple_scenarios(models, all_hosts, configuration_traces_files, malicious_traces_files, lsh=True):
    ...
    for i, host in enumerate(all_hosts):
        ...
        for i, model in enumerate(models):
            ...
            #     break
        if np.sum(evaluation_results) >= len(evaluation_results) // 2:
            # if malicious_found:
            evaluation_result = 1
        else:
            evaluation_result = 0
...
def run_single_scenario_evaluation(lsh=True):
    ...
    window = '100median'
    stride = '50median'
    rootdir = '../data/discretized_data_{}_{}/ctu_13/single_scenario/{}'.format(window, stride, features_dir)
    # rootdir = '../data/discretized_data_{}/ctu_13/single_scenario'.format('20ms')
...
def run_multiple_scenarios_evaluation(lsh=True):
    ...
    window = '100median'
    stride = '50median'
    # rootdir = '../data/discretized_data_{}/ctu_13/multiple_scenarios'.format('20ms')
    rootdir = '../data/discretized_data_{}_{}/ctu_13/multiple_scenarios/{}'.format(window, stride, features_dir)

...
from pypackage.flexfringe.utils import split_data
...
def write_to_csv(results, klthreshold, states, case, lsh=True):
    ...
    if lsh:
        file_workbook = "../results/ctu_connection/ctu_connection_0.1_lsh_results_{}_{}_{}.xlsx".format(window, stride, case)
    else:
        file_workbook = "../results/ctu_connection/ctu_connection_trigram_results_{}_{}_{}.xlsx".format(window, stride, case)
...
def evaluate_multiple_scenarios(models, all_hosts, configuration_traces_files, malicious_traces_files, lsh=True):
    ...
    finger_eval = get_evaluation_metrics(host_labels_fingerprint, predicted_labels_fingerprint)
    results += [list(finger_eval)]
    write_to_csv(results, 0, 0, 'multiple_fixed_100', lsh=lsh)
...
def run_multiple_scenarios_evaluation(lsh=True):
    ...
    global data_dir
    global scenario
    rootdir = "../data/discretized_data_fixed_100_flows/ctu_13/connection/multiple_scenarios"

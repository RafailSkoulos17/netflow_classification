...
selected_features = ['duration', 'direction', 'protocol', 'packets', 'src_bytes', 'dst_bytes', 'time_diff']
...
def write_to_csv(results, klthreshold, states, case, lsh=True):
    pre = "median" if "median" in window else ""
    workbook_path = "../results/ctu/{}/connection".format(features_dir)
    if not os.path.exists(workbook_path):
        os.makedirs(workbook_path)
    if lsh:
        file_workbook = workbook_path + "/3LSH_RBS_{}_ctu_connection_0.01_lsh_results_{}_{}_{}.xlsx".format(
            pre, window,
            stride,
            case)
    else:
        file_workbook = workbook_path + "/{}_ctu_connection_trigram_results_{}_{}_{}.xlsx".format(
            pre, window,
            stride,
            case)
...
def train_model(malicious_traces_file, difference):
    ...
    parser.add_argument('--mode', '-M', help="batch or stream depending on the mode of operation", type=str, default='batch')
    parser.add_argument('--heuristic_name', '-hn', help="The Name of the heuristic", type=str, default='lsh3')
    parser.add_argument('--data_name', '-d', help="Type of data for the heuristic", type=str, default='lsh3_data')
    ...
    parser.add_argument('--tablesize', '-S', help="tablesize", type=str, default='100')
    ...
    parser.add_argument('--binarycodebytes', '-c', help="binarycodebytes", type=str, default='10')
    ...
    states = flexfringe(args.train_file, M=args.mode, h=args.heuristic_name, d=args.data_name, q=args.state_count,
                        y=args.symbol_count, f=args.finalred,
...
def evaluate_single_scenario(all_connections, malicious_traces_file, configuration_traces_files, lsh=True):
    if lsh:
    ...
    all_connections = [c for c in all_connections if not c.endswith('_dir')]

    connection_classification = {}
    connection_classification['prof'] = {}
    connection_classification['error'] = {}
    connection_classification['finger'] = {}
    results = []
    connection_labels_prof = []
    ...
    for connection in all_connections:
        if 'benign' in connection:
            connection_label = 0
        else:
            connection_label = 1
        evaluation_result = profiling_evaluation(model, connection, acceptance_ratio)
        if evaluation_result == connection_label:
            connection_classification['prof'][connection] = True
        else:
            connection_classification['prof'][connection] = False
        connection_labels_prof += [connection_label]
        predicted_labels_prof += [evaluation_result]
    print('Profiling Classification')
    prof_eval = get_evaluation_metrics(connection_labels_prof, predicted_labels_prof)
    results += [list(prof_eval)]

    threshold = get_selectivity_threshold(model, configuration_traces_files, malicious_traces_file)

    connection_labels_err = []
    ...
    for connection in all_connections:
        if 'benign' in connection:
            ...
            connection_label = 1
        evaluation_result = error_based_classification(model, connection,
                                                       malicious_traces_file, threshold)
        if evaluation_result == connection_label:
            connection_classification['error'][connection] = True
        else:
            connection_classification['error'][connection] = False
        connection_labels_err += [connection_label]
        predicted_labels_err += [evaluation_result]
    print('\nError Based Classification')
    error_eval = get_evaluation_metrics(connection_labels_err, predicted_labels_err)
    results += [list(error_eval)]

    connection_labels_fingerprint = []
    ...
    for connection in all_connections:
        if 'benign' in connection:
            ...
            connection_label = 1
        evaluation_result = fingerprint_based_classification(model, connection,
                                                             malicious_traces_file, configuration_traces_files)
        if evaluation_result == connection_label:
            connection_classification['finger'][connection] = True
        else:
            connection_classification['finger'][connection] = False
        connection_labels_fingerprint += [connection_label]
        predicted_labels_fingerprint += [evaluation_result]
    print('\nFingerprint Based Classification')
    finger_eval = get_evaluation_metrics(connection_labels_fingerprint, predicted_labels_fingerprint)
...
def evaluate_multiple_scenarios(models, all_connections, configuration_traces_files, malicious_traces_files, lsh=True):
    # host_classification = {}
    ...
    for i, connection in enumerate(all_connections):
        ...
        print('Profiling Classification: {} of {}'.format(i + 1, len(all_connections)))
        if 'benign' in connection:
            connection_label = 0
        else:
            connection_label = 1
        ...
        for i, model in enumerate(models):
            r = profiling_evaluation(model, connection, acceptance_ratio[i])
    ...
    for i, connection in enumerate(all_connections):
        malicious_found = False
        print('Error Based Classification: {} of {}'.format(i + 1, len(all_connections)))
        if 'benign' in connection:
            connection_label = 0
        else:
            connection_label = 1
        evaluation_results = []
        for i, model in enumerate(models):
            r = error_based_classification(model, connection, malicious_traces_files[i], threshold[i])
            evaluation_results += [r]
            if r == 1:
                malicious_found = True
                break
    ...
    for i, connection in enumerate(all_connections):
        malicious_found = False
        print('Fingerprint Based Classification: {} of {}'.format(i + 1, len(all_connections)))
        if 'benign' in connection:
            connection_label = 0
        else:
            connection_label = 1
        evaluation_results = []
        for i, model in enumerate(models):
            r = fingerprint_based_classification(model, connection, malicious_traces_files[i],
...
def run_single_scenario_evaluation(lsh=True):
    ...
    window = '100median'
    stride = '50median'
    rootdir = '../data/discretized_data_{}_{}/ctu_13/connection/single_scenario/{}'.format(window, stride, features_dir)
    for subdir, dirs, files in os.walk(rootdir):
        ...
        malicious_traces_files = [f for f in all_connections if ('infected' in f and not f.endswith("_dir"))]
        benign_traces_files = [f for f in all_connections if 'benign' in f]
        configuration_traces_files = [f for f in all_connections if 'configuration' in f]
        if not malicious_traces_files:
            continue
        else:
            malicious_traces_file = malicious_traces_files[0]
        # benign_traces_file = benign_traces_files[0]
        all_connections.remove(malicious_traces_file)
        for conf_host in configuration_traces_files:
            all_connections.remove(conf_host)
        # all_hosts.remove(benign_traces_file)
        evaluate_single_scenario(all_connections, malicious_traces_file, configuration_traces_files, lsh=lsh)
...
def run_multiple_scenarios_evaluation(lsh=True):
    ...
    window = '100median'
    stride = '50median'
    rootdir = '../data/discretized_data_{}_{}_notall_direction_timediff/ctu_13/connection/multiple_scenarios'.format(
        window, stride)

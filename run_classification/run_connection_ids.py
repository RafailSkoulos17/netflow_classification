...
def flexfringe(*args, **kwargs):
    ...
    #     print(line.rstrip())
    dfa_path = args[0] + '_dir/final.dot'
    try:
        with open(dfa_path) as fh:
            # return fh.read()
            return states
...
def write_to_csv(results, klthreshold, states, case, lsh=True):
    if lsh:
        file_workbook = "../results/ids_connection/ids_connection_0.1_lsh_results_{}_{}_{}.xlsx".format(window, stride,
                                                                                                        case)
    else:
        file_workbook = "../results/ids_connection/ids_connection_trigram_results_{}_{}_{}.xlsx".format(window, stride,
                                                                                                        case)
...
def train_model(malicious_traces_file, difference):
    ...
    parser.add_argument('--klthreshold', '-T', help="klthreshold", type=str, default='0.05')
...
def evaluate_single_scenario(all_hosts, malicious_traces_file, configuration_traces_files, lsh=True):
    if lsh:
        ...
        # model = '/'.join(malicious_traces_file.split('/')[:-1]) + '/final.dot'
...
def run_single_scenario_evaluation(lsh=True):
    ...
    global data_dir
    global scenario
    rootdir = "../data/discretized_data/ids/connection/single_scenario/"
    # for subdir, dirs, files in os.walk(rootdir):
    #     if len(subdir.split('/')) != 5:
    #         continue
    # used_dirs = ['Wednesday-workingHours.pcap_ISCX', 'Tuesday-WorkingHours.pcap_ISCX',
    #              'Friday-WorkingHours-Morning.pcap_ISCX', 'Friday-WorkingHours-Afternoon-DDos.pcap_ISCX']

    for dir in os.listdir(rootdir):
        # if dir in used_dirs:
        #     continue
        dir_files = os.listdir(os.path.join(rootdir, dir))
        for item in dir_files:
            if os.path.isdir(item) and item != 'configuration_data':
                shutil.rmtree(item)
        #     dir = [d for d in dirs if 'ctu' in d][0]
        # dir = [d for d in dirs if '20ms-1 flow' in d][0]
        scenario = dir.split('/')[-1]
        ...
        window = '1000median'
        stride = '500median'
        data_dir = os.path.join(rootdir, dir)
        all_connections = glob.glob(data_dir + "/*traces".format(scenario))
        conf_hosts = glob.glob(data_dir + "/configuration_data" + "/*".format(scenario))
        malicious_traces_files = [f for f in all_connections if 'infected' in f]
        benign_traces_files = [f for f in all_connections if 'benign' in f]
        configuration_traces_files = [f for f in conf_hosts]
        if len(malicious_traces_files) >= 1:
...
def run_multiple_scenarios_evaluation(lsh=True):
    ...
    global data_dir
    global scenario
    rootdir = "../data/discretized_data_100_50/ids/connection/multiple_scenarios"

    models = []
    given_malicious_trace = []
    conf_hosts = glob.glob(rootdir + '/configuration_data/*')
    configuration_traces_files = [f for f in conf_hosts]
    malicious_traces_files = {}
    for subdir, dirs, files in os.walk(rootdir):
        if len(subdir.split('/')) != 7 and subdir.split('/')[-1] != 'configuration_data':
            continue
        sheet_name = subdir.split('/')[-1]
        all_connections = glob.glob(subdir + "/*traces")
        malicious_traces_files[sheet_name] = [f for f in all_connections if 'infected' in f]
    ...
    for subdir, dirs, files in os.walk(rootdir):
        if len(subdir.split('/')) != 7 and subdir.split('/')[-1] != 'configuration_data':
            continue
        sheet_name = subdir.split('/')[-1]
        if len(malicious_traces_files[sheet_name]) < 1:
            continue
        current_malicious_traces_files = [malicious_traces_files[sheet_name][0]]

        for malicious_traces_file in current_malicious_traces_files:
            if lsh:
                with open(malicious_traces_file, 'r') as f:
                    i = f.readline().split()[1]
                    klthreshold, states = train_model(malicious_traces_file, str(int(i) + 10))
                model = malicious_traces_file + '_dir/final.dot'
            else:
                model = train_trigram_model(malicious_traces_file)
                klthreshold, states = 0, 0
    ...
    for subdir, dirs, files in os.walk(rootdir):
        if len(subdir.split('/')) != 7 or subdir.split('/')[-1] == 'configuration_data':
            continue
        sheet_name = subdir.split('/')[-1]
        scenario = sheet_name
        window = 100
        ...
        stride = str(stride) + 'median'
        if len(malicious_traces_files[sheet_name]) <= 1:
            all_connections = [f for f in glob.glob(subdir + "/*traces") if 'benign' in f]
        else:
            all_connections = [f for f in glob.glob(subdir + "/*traces") if 'benign' in f] + malicious_traces_files[
                                                                                                 sheet_name][1:]
        evaluate_multiple_scenarios(models, all_connections, configuration_traces_files, given_malicious_trace, lsh=lsh)
